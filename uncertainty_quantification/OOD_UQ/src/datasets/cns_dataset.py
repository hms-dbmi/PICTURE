...
class CNSDataset(Dataset):
    def __init__(self, slides_file="/n/data2/hms/dbmi/kyu/lab/jz290/BRAIN_20X/", transform=None, target_transform=None, dino=False, normalize_stain = False, label_map = None):
    ...
    def __getitem__(self, idx):
        ...
        return {"image": patch, "label": label_to_use, "idx": idx, "label_name": self.reduced_labels[label], "patient_id": self.dict_path_labels[idx]["patient_id"]}

    def find_jpeg(self, path, labels: list = ["Ebrain_OOD"]):

        # Find all the jpeg files in GBM and PCNL folders
        dict_path_labels = []

        for label_int, label_name in enumerate(labels):
            for root, dirs, files in os.walk(os.path.join(path, label_name)):
                for file in files:
                    if file.endswith(".jpg"):
                        dict_path_labels.append({"paths": os.path.join(root, file), "coarse_labels": label_int, "patient_id": os.path.splitext(os.path.basename(root))[0]})
