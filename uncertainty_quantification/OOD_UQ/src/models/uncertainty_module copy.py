...
class UncertaintyModule(LightningModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        ood_datamodule = EbrainDataModule(batch_size=batch_size, num_workers=4, extra_ood=extra_ood)
        ...
        self.net = NaturalPosteriorNetworkModel(
            ...
            flow = RadialFlow(self.hparams["latent_dim"], num_layers=self.hparams["radial_flow_layers"]),
    ...
    def validation_epoch_end(self, outputs: List[Any]):
        ...
        if self.current_epoch >= 0:
            (aleatoric_confidence_auc_pr, aleatoric_confidence_auc_roc, epistemic_confidence_auc_pr, epistemic_confidence_auc_roc) = self.compute_auc_roc(self.val_ood_dataset,output_tsne=self.hparams.output_val_tsne)
    ...
    def test_epoch_end(self, outputs: List[Any]):
        ...
        if not hasattr(self,"test_ood_dataset"):
            ...
            ood_dataset = self.ood_datamodule.test_dataloader().dataset
            # min_len = min(len(id_dataset), len(ood_dataset))
            # id_dataset = torch.utils.data.Subset(id_dataset, range(min_len))
            # ood_dataset = torch.utils.data.Subset(ood_dataset, range(min_len))
        ...
        # log calibration metrics
        (aleatoric_confidence_auc_pr, aleatoric_confidence_auc_roc, epistemic_confidence_auc_pr, epistemic_confidence_auc_roc) = self.compute_auc_roc(self.test_ood_dataset,output_tsne=True)
    ...
    def compute_epistemic_aleatoric(self, ood_dataset, limit = 300,output_tsne=False):
        """
...
#        self.logger.experiment.log({"roc_curve_epistemic": wandb.Image(plt)})
    ...
    def compute_auc_roc(self, ood_dataset,output_tsne=False):
        """
        Compute the AUC-ROC values for aleatoric and epistemic confidence on the given out-of-distribution dataset using
        the given model.
        ...
        """
        y_true, y_score_epistemic, y_score_aleatoric = self.compute_epistemic_aleatoric(ood_dataset,output_tsne=output_tsne)

        # Compute AUC-PR and AUC-ROC for aleatoric and epistemic confidence
        aleatoric_confidence_auc_pr = AUCPR(compute_on_step=False,task='binary')( torch.tensor(y_score_aleatoric),torch.tensor(y_true))
        aleatoric_confidence_auc_roc = AUROC(compute_on_step=False,task='binary')( torch.tensor(y_score_aleatoric),torch.tensor(y_true))
        epistemic_confidence_auc_pr = AUCPR(compute_on_step=False,task='binary')( torch.tensor(y_score_epistemic),torch.tensor(y_true))
        epistemic_confidence_auc_roc = AUROC(compute_on_step=False,task='binary')( torch.tensor(y_score_epistemic),torch.tensor(y_true))
    ...
    def __init__(
        ...
        self.net = NaturalPosteriorNetworkModel(
            ...
            flow=RadialFlow(
                ...
                num_layers=self.hparams["radial_flow_layers"],
            ),
            output=classifier
        )
        ...
        # to ensure a proper reduction over the epoch
        task_type='binary'
        self.train_acc = Accuracy(multidim_average="global",task=task_type)
        self.val_acc = Accuracy(multidim_average="global",task=task_type)
        self.test_acc = Accuracy(multidim_average="global",task=task_type)
        ...
        self.alea_conf_pr = AUCPR(compute_on_step=False, dist_sync_fn=self.all_gather,task=task_type)
        self.alea_conf_roc = AUROC(compute_on_step=False, dist_sync_fn=self.all_gather,task=task_type)

        self.epist_conf_pr = AUCPR(compute_on_step=False, dist_sync_fn=self.all_gather,task=task_type)
        self.epist_conf_roc = AUROC(compute_on_step=False, dist_sync_fn=self.all_gather,task=task_type)
